import { db } from "@/lib/prisma";
import { inngest } from "./client";
import { GoogleGenerativeAI } from "@google/generative-ai";

const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash" });

export const generateIndustryInsights = inngest.createFunction(
  { name: "Generate Industry Insights" },
  { cron: "0 0 * * 0" }, // Run every Sunday at midnight
  async ({ event, step }) => {
    const industries = await step.run(
      "Fetch industries with users",
      async () => {
        return await db.industryInsight.findMany({
          select: {
            id: true,
            industry: true,
            userId: true,
            nextUpdate: true,
          },
        });
      }
    );

    for (const insight of industries) {
      // Only update insights that are due for an update
      if (!insight.nextUpdate || new Date() > new Date(insight.nextUpdate)) {
        const prompt = `
            Analyze the current state of the ${insight.industry} industry and provide insights in ONLY the following JSON format without any additional notes or explanations:
            {
              "salaryRanges": [
                { "role": "string", "min": number, "max": number, "median": number, "location": "string" }
              ],
              "growthRate": number,
              "demandLevel": "High" | "Medium" | "Low",
              "topSkills": ["skill1", "skill2"],
              "marketOutlook": "Positive" | "Neutral" | "Negative",
              "keyTrends": ["trend1", "trend2"],
              "recommendedSkills": ["skill1", "skill2"]
            }
            
            IMPORTANT: Return ONLY the JSON. No additional text, notes, or markdown formatting.
            Include at least 5 common roles for salary ranges.
            Growth rate should be a percentage.
            Include at least 5 skills and trends.
          `;

        try {
          const res = await step.ai.wrap(
            "gemini",
            async (p) => {
              return await model.generateContent(p);
            },
            prompt
          );

          const text = res.response.text();
          const cleanedText = text.replace(/```(?:json)?\n?|```$/g, "").trim();

          try {
            const newInsights = JSON.parse(cleanedText);

            await step.run(
              `Update ${insight.industry} insights for user ${insight.userId}`,
              async () => {
                await db.industryInsight.update({
                  where: { id: insight.id },
                  data: {
                    ...newInsights,
                    lastUpdated: new Date(),
                    nextUpdate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000),
                  },
                });
              }
            );
          } catch (parseError) {
            console.error(
              `Failed to parse insights for ${insight.industry}:`,
              parseError
            );
            console.error("Raw text:", cleanedText);
          }
        } catch (aiError) {
          console.error(
            `AI generation failed for ${insight.industry}:`,
            aiError
          );
        }
      }
    }
  }
);
